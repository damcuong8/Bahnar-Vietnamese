...
class ViBaSpeechToTextDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        vi_tokens = None
        vi_attention_mask = None
        if vi_text:
            vi_encoded = self.processor(
                text=vi_text,
                return_tensors="pt",
                padding=False,
                truncation=True,
            ...
            vi_tokens = vi_encoded["input_ids"].squeeze(0)  # Shape: [seq_len]
            vi_attention_mask = vi_encoded["attention_mask"].squeeze(0)  # Shape: [seq_len]
        
        # Tokenize English text
        en_tokens = None
        en_attention_mask = None
        if en_text:
            en_encoded = self.processor(
                text=en_text,
                return_tensors="pt",
                padding=False,
                truncation=True,
            ...
            en_tokens = en_encoded["input_ids"].squeeze(0)  # Shape: [seq_len]
            en_attention_mask = en_encoded["attention_mask"].squeeze(0)  # Shape: [seq_len]
...
class DataCollatorSpeechToText:
    ...
    def __call__(self, features: List[Dict[str, Any]]) -> Dict[str, torch.Tensor]:
        ...
        # This handles: mel-spectrogram extraction, normalization, padding
        ...
        if self.target_language == "vi":
            target_ids = [f["vi_input_ids"] for f in features if f["vi_input_ids"] is not None]
        else:
            target_ids = [f["en_input_ids"] for f in features if f["en_input_ids"] is not None]
        ...
        if target_ids:
            ...
            # Replace padding token ids with -100 so they are ignored by loss function
            mask = labels_batch["attention_mask"].eq(1)
            labels = torch.where(mask, labels_batch["input_ids"], torch.tensor(-100))

            batch["labels"] = labels
        
        # Process pivot language tokens (for context/conditioning)
        if self.pivot_language == "en":
            pivot_ids = [f["en_input_ids"] for f in features if f["en_input_ids"] is not None]
        else:
            pivot_ids = [f["vi_input_ids"] for f in features if f["vi_input_ids"] is not None]
        
        if pivot_ids:
            # Pad pivot sequences
            pivot_batch = self.processor.tokenizer.pad(
                {"input_ids": pivot_ids},
                padding=self.padding,
                return_tensors="pt"
            )
...
def main():
    ...
    data_collator = DataCollatorSpeechToText(feature_extractor=feature_extractor, processor=processor, padding=True, pad_to_multiple_of=8, target_language="vi", pivot_language="en")
